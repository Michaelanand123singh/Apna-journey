...
import { Briefcase, MapPin, Clock, Building, ArrowRight } from 'lucide-react'
...
export default function LatestJobs() {
  ...
  const fetchLatestJobs = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/jobs?limit=10&status=approved')
        ...
        <div className="space-y-2">
          {jobs.map((job) => (
            <Link
              key={job._id}
              href={`/jobs/${job.slug}`}
              className="group block bg-white rounded-lg border border-gray-200 hover:border-green-300 hover:shadow-md transition-all duration-200"
            >
              <div className="p-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <span className="bg-green-100 text-green-800 text-[9px] font-medium px-2 py-0.5 rounded whitespace-nowrap">
                      {job.jobType.replace('-', ' ')}
                    </span>
                    {job.salary && (

...
import { Laptop, MapPin, Clock, ArrowRight, Code } from 'lucide-react'
import { Job } from '@/types'

export default function ITSoftwareJobs() {
  ...
  const fetchITJobs = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/jobs?category=it&limit=100&status=approved')
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-1">
            ...
            <h2 className="text-lg sm:text-xl font-bold text-gray-900">IT & Software</h2>
          </div>
          <p className="text-xs text-gray-600 pl-7">Developer, QA, DevOps and more</p>
        </div>
        <Link
      ...
      {jobs.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-xl">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-2">No IT & Software Jobs</h3>

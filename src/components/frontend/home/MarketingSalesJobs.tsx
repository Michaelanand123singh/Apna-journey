...
import { Megaphone, MapPin, Clock, ArrowRight, DollarSign } from 'lucide-react'
import { Job } from '@/types'

export default function MarketingSalesJobs() {
  ...
  const fetchMarketingJobs = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/jobs?category=marketing&limit=100&status=approved')
      const data = await response.json()
      if (data.success) {
        setJobs(data.data)
      }
    } catch (error) {
      console.error('Error fetching Marketing & Sales jobs:', error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-1">
            ...
            <h2 className="text-lg sm:text-xl font-bold text-gray-900">Marketing & Sales</h2>
          </div>
          <p className="text-xs text-gray-600 pl-7">Sales, marketing, growth roles</p>
        </div>
        <Link
      ...
      {jobs.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-xl">
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-2">No Marketing & Sales Jobs</h3>
